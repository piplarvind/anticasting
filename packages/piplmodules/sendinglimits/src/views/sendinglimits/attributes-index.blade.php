    ...
    <div class="content-body">
        <div class="main-view-content">
            <div class="all-ourt-style w-80-cust">
                <div class="all-heads">
                    <h3>Sending Limits Attributes</h3>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="{{ url('/') }}/admin"> <i class="fa fa-tachometer"></i> Dashboard</a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="{{ route('admin.sendlimits') }}">Sending Limits</a>
                        </li>
                        <li class="breadcrumb-item active">
                            Sending Limits Attributes
                        </li>
                    </ol>
                </div>
                <div class="card">
                    <div class="card-body">
                        ...
                        {{-- <div class="pull-right">
                            <a class="btn btn-success pull-right" style="margin-bottom: 5px;"
                                href="{{ route('admin.sendlimits.attributes.create', [$id]) }}">Add Attribute</a>
                        </div> --}}
                                    ...
                                    @if (isset($items) && $items->count())
                                        @foreach ($items as $k => $item)
                                            <tr class="@if ($k % 2 == 0) even @else odd @endif pointer"
                                                ...
                                                <td>{{ $item->one_day_price }}</td>
                                                <td>{{ $item->thirty_day_price }}</td>
                                                <td>{{ $item->half_yearly_price }}</td>
                                                ...
                                                <td>
                                                    <span class="act-link">
                                                        <a class="btn btn-primary shadow btn-xs sharp mr-1"
                                                            href="{{ route('admin.sendlimits.attributes.edit', [$tier->id, $item->id]) }}"
                                                            data-toggle="tooltip" data-placement="top" title=""
                                                            data-original-title="Edit"><i class="fa fa-pencil"></i></a>
                                                    </span>
                                                    {{-- <span class="act-link">
                                                        <a class="btn btn-danger shadow btn-xs sharp"
                                                            href="{{ route('admin.sendlimits.attributes.delete', [$tier->id, $item->id]) }}"
